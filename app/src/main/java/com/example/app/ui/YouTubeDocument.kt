...
internal fun youTubeDocument(id: String, positionMs: Long, rate: Float, play: Boolean, origin: String): String {
    ...
    return """
        ...
        var requestedVideo='$id', requestedStart=$start, requestedPlay=$play, requestedRate=$speed, playerReady=false, awaitingRequested=true;
        function loadRequestedVideo(id,start){requestedVideo=id;requestedStart=start;requestedPlay=true;awaitingRequested=true;if(playerReady){player.loadVideoById(id,start);player.setPlaybackRate(requestedRate);}}
        var player, reportedSelection=''; function reportPlayback(){
        if(player&&player.getVideoData){var selected=player.getVideoData().video_id;if(awaitingRequested){if(selected!==requestedVideo)return;awaitingRequested=false;}if(selected&&selected!==requestedVideo){if(selected!==reportedSelection){reportedSelection=selected;Companion.selectedVideo(selected);}return;}reportedSelection='';}
        if(player&&player.getPlayerState&&player.getCurrentTime)Companion.playback(player.getPlayerState(),Math.floor(player.getCurrentTime()),Math.floor(player.getDuration()||0));}
        ...
        setInterval(function(){reportPlayback();if(!awaitingRequested&&!reportedSelection&&player&&player.getPlayerState&&player.getPlayerState()===1)Companion.position(Math.floor(player.getCurrentTime()));},1000);
        </script></body></html>
    """.trimIndent()
}
