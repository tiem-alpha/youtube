...
fun ShortsScreen(vm: VideoViewModel, modifier: Modifier, open: (VideoResult) -> Unit) {
    ...
    val pager = rememberPagerState { state.videos.size }
    LaunchedEffect(pager.settledPage, state.videos.size) { if (state.videos.isNotEmpty() && pager.settledPage >= state.videos.lastIndex - 2) vm.more() }
    Column(modifier) {
        ...
        if (state.videos.isEmpty()) {
        ...
        } else VerticalPager(pager, Modifier.weight(1f), key = { state.videos[it].id }) { index ->
            ...
            Column(Modifier.fillMaxSize()) {
                ...
                Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                    ...
                    if (index == state.videos.lastIndex && state.message != null) TextButton(vm::retry) { Text("Thử tải thêm") }
                }
            }
        }
    }
