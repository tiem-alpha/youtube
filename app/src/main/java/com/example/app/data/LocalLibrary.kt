...
data class LibraryState(val history: List<SavedVideo> = emptyList(), val watchLater: List<VideoResult> = emptyList(), val playlists: List<LocalPlaylist> = emptyList(), val searches: List<String> = emptyList(), val hiddenIds: Set<String> = emptySet())
...
class LocalLibrary(context: Context, preferencesName: String = "library") {
    ...
    fun importSearches(queries: List<String>) {
        save(_state.value.copy(searches = mergeSearches(_state.value.searches, queries)))
    }
    ...
    companion object {
        ...
        fun mergeSearches(current: List<String>, legacy: List<String>): List<String> =
            (current + legacy).map(String::trim).filter(String::isNotBlank)
                .distinctBy { it.lowercase(java.util.Locale.ROOT) }.take(20)

        private fun videoJson(v: VideoResult) = JSONObject().put("id", v.id).put("title", v.title).put("channel", v.channel).put("thumbnail", v.thumbnailUrl).put("channelId", v.channelId).put("description", v.description).put("publishedAt", v.publishedAt).put("duration", v.duration).put("views", v.views).put("live", v.live).put("categoryId", v.categoryId).put("isShort", v.isShort)
        private fun video(j: JSONObject) = VideoResult(j.getString("id"), j.optString("title"), j.optString("channel"), j.optString("thumbnail").takeIf { it.isNotBlank() && it != "null" }, j.optString("channelId"), j.optString("description"), j.optString("publishedAt"), j.optString("duration"), j.optString("views"), j.optBoolean("live"), categoryId = j.optString("categoryId"), isShort = j.optBoolean("isShort"))
        ...
        fun encode(state: LibraryState): String = JSONObject()
            ...
            .put("hiddenIds", JSONArray(state.hiddenIds.toList()))
        ...
        fun decode(raw: String?): LibraryState = runCatching {
            ...
            LibraryState(objects(j.optJSONArray("history")).mapNotNull { runCatching { SavedVideo(video(it.getJSONObject("video")), it.optInt("position").coerceAtLeast(0), it.optLong("updated")) }.getOrNull() },
                ...
                j.optJSONArray("hiddenIds")?.let { a -> (0 until a.length()).map { a.optString(it) }.filter(String::isNotBlank).toSet() } ?: emptySet())
